...
void object_draw_colored(Object *object, mat4_t *mat, rgba_t color) {
	...
	// TODO: check for PRM_SINGLE_SIDED
	// ship engine (wipeout1): FT3, GT3, and flag is PRM_SHIP_ENGINE
				...
				.vertices = {
					{
						.pos = vertex[coord2],
						.uv = {poly.gt3->u2, poly.gt3->v2},
						.color = flags_is(poly.gt3->flag, PRM_SHIP_ENGINE) ? poly.gt3->color[2] : color
					},
					{
						.pos = vertex[coord1],
						.uv = {poly.gt3->u1, poly.gt3->v1},
						.color = flags_is(poly.gt3->flag, PRM_SHIP_ENGINE) ? poly.gt3->color[1] : color
					},
					{
						.pos = vertex[coord0],
						.uv = {poly.gt3->u0, poly.gt3->v0},
						.color = flags_is(poly.gt3->flag, PRM_SHIP_ENGINE) ? poly.gt3->color[0] : color
					},
				}
			}, poly.gt3->texture);
				...
				.vertices = {
					{
						.pos = vertex[coord2],
						.uv = {poly.ft3->u2, poly.ft3->v2},
						.color = flags_is(poly.ft3->flag, PRM_SHIP_ENGINE) ? poly.ft3->color : color
					},
					{
						.pos = vertex[coord1],
						.uv = {poly.ft3->u1, poly.ft3->v1},
